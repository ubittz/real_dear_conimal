...
class DataUtils {
  ...
  static List<int> getRandomThreeInt({
    required int dividerNumber,
  }) {
    final randomObj = Random();
    final firstNumber = randomObj.nextInt(dividerNumber);
    int secondNumber = randomObj.nextInt(dividerNumber);
    int thirdNumber = randomObj.nextInt(dividerNumber);

    while (firstNumber == secondNumber) {
      secondNumber = randomObj.nextInt(dividerNumber);
    }
    while (firstNumber == thirdNumber || secondNumber == thirdNumber) {
      thirdNumber = randomObj.nextInt(dividerNumber);
    }
    final returnList = [firstNumber, secondNumber, thirdNumber];
    returnList.sort();
    return returnList;
  }
  ...
  static List<T> sort<T>(List<T> items, int Function(T a, T b) compare) {
    List<T> newList = List.from(items); // 원본 리스트의 복사본 생성
    newList.sort((a, b) => compare(a, b)); // 복사본 리스트를 정렬
    return newList; // 섞인 리스트 반환
  }
